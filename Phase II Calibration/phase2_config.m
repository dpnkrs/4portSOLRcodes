...
config.reference_standard_mode = 'stitched_final';
config.reference_bandwise_field = 'gamma_winner';
config.transmission_anchor = struct( ...
    'enabled', false, ...
    'bands', {{}}, ...
    'mode', 'phase1_complex_ratio', ...
    'trim_frac', 0.15, ...
    'max_gain_db', 6, ...
    'min_target_mag_db', -40);
config.reciprocal_target_normalization = struct( ...
    'enabled', false, ...
    'band', '', ...
    'ratio', NaN);
config.branch_stabilization = struct( ...
    'enabled', true, ...
    'target_weight', 1.0, ...
