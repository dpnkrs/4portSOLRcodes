...
lines{end + 1} = sprintf('4. Parse the four wave CSV files for that pair, validate the filename semantics against the active pair, and compute forward/reverse switch ratios using %s.', config.switch_ratio_mode);
...
lines{end + 1} = 'The switch-term loader enforces the pair semantics a_jS_i / b_jS_i for forward and a_iS_j / b_iS_j for reverse before any ratios are formed.';
lines{end + 1} = '';
